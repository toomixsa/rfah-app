...
CMD ["bash","-lc","mkdir -p /app/instance /app/src/instance /app/data && echo Using DB: ${SQLALCHEMY_DATABASE_URI:-$DATABASE_URL} && exec gunicorn -w ${GUNICORN_WORKERS:-1} -b 0.0.0.0:${PORT} src.main:app"]
